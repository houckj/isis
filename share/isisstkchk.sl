...
private define _slstkchk_main ()
{
...
#ifexists stkcheck_set_output_hook
	stkcheck_set_output_hook (&_slstkchk_output_hook, fp);
#else
        vmessage ("*** Sorry, the file output option requires _slang_version >= 2.2.4");
#endif
     }
   ...
   if (ref == NULL && main == "slsh_main")
     {
        ref = __get_reference ("isis_main");
        if (ref == NULL)
          () = fprintf (stderr, "*** Warning both slsh_main and isis_main are undefined\n");
     }
