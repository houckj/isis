...
Isis provides a number of optimization methods. Note that the
methods have different strengths and weaknesses and are by no
means equivalent.  In the general case, multidimensional
nonlinear optimization problems are extremely challenging.
Because no single method works well on every problem, it is
often necessary to take care in choosing which optimization
method to use for a given problem.

When the problem of interest is well behaved in the sense that
the statistic is a smoothly varying function of a small number
of uncorrelated parameters and has a unique minimum whose
location can be guessed with reasonable accuracy, then a local
optimization method such as Levenberg-Marquardt is most likely
to perform well, in the sense of accurately locating the
minimum after a relatively small number of model evaluations
(see \verb|lmdif|, \verb|marquardt|, \verb|plm|).  The Powell
method may also work well in such cases (see \verb|powell|).

For less ideal problems, the simplex method (see
\verb|simplex|, \verb|subplex|) or Powell method (see
\verb|powell|) may be more effective, although significantly
more model evaluations may be required.

For the most difficult problems, a global method such as
differential evolution (\verb|diffevol|) or simulated annealing
(\verb|simann|) may yield useful results while the other
methods might make little or no progress.  However, the large
number of model evaluations required by these methods makes
them very CPU intensive.
...
{optimization, set\_fit\_method, multicore}
