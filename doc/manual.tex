...
{plasma, db\_push, db\_status, db\_select}
...
\begin{isisfunction}
{db\_status}%
{list of currently loaded spectroscopy databases}%
{db\_status ([Ref\_Type | File\_Type])}% usage
{db\_push, db\_select, plasma, atoms}
...
{db\_status, db\_select, plasma, atoms}

Use this function to load another spectroscopy database. The
\verb|Struct_Type| argument has the same form as the one used 
as input to the \verb|plasma| function.
...
{db\_push, db\_status, plasma, atoms}

Use this function to select a spectroscopy database from the currently list
displayed by \verb|db_status|.  Subsequent calls to database access functions
