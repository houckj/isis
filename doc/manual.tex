...
{define\_back, \_define\_back, back\_fun, get\_back\_data, get\_back\_model,
get\_back\_data\_scale\_factor}

Returns an array of instrumental background values.  If no \verb|back_fun| has
been specified, the scaled instrumental background is returned, otherwise the
result of the specified background model is returned. These values are on the
same wavelength grid as that returned by e.g. \verb|get_data_counts|.
\end{isisfunction}

\begin{isisfunction}
{get\_back\_data}
{Retrieve the un-scaled instrumental background for a given spectrum}
{b = get\_back\_data (hist\_index)}
{define\_back, \_define\_back, get\_back\_data\_scale\_factor, get\_back\_model, get\_back}
...
{scale = get\_back\_data\_scale\_factor (hist\_index)}
{define\_back, \_define\_back, get\_back\_data, get\_back\_model, get\_back}
...
\end{verbatim}
The \verb|exposure| time value used for the data is normally taken from the
ARF.  If the ARF exposure is unspecified or invalid, the data exposure value
is used.  Either \verb|backscale| value may be a vector quantity, defined on
the same wavelength grid as the data.
...
{b = get\_back\_model (hist\_index)}
{back\_fun, define\_back, \_define\_back, get\_back\_data\_scale\_factor, get\_back}
...
{fit\_fun, set\_par, flux\_corr, define\_back, \_define\_back,
get\_back\_model, get\_back}
...
By default, isis will generate an error if asked to load a
FITS RMF file that does not adhere closely to the OGIP
standard format.  Setting the global variable \verb|Rmf_OGIP_Compliance=0|
will reduce the required level of standards compliance for all
input RMF files.  Alternatively, one can use the \verb|strict|
qualifier to change the setting for a specific file. For example:
\begin{verbatim}
   rmf_index = load_rmf ("non_ogip_compliant.rmf;strict=0");
\end{verbatim}

If isis is still unable to read the file, the best approach may
be to modify the RMF file to adhere more closely to the standard
...
or if the qualifier \verb|strict=0| is present.
...
(up\_name/lo\_name) if available.  For each energy level, the level
degeneracy (g), excitation energy (E) and LS coupling quantum numbers
are also provided.  The ionization state is the integer
...
{build\_xspec\_local\_models ([dir [, pkg\_name]][;lmodel=filename])};
...
path and a package name may be provided.  By default, model parameter
names are loaded from a text file named \verb|lmodel.dat|; an alternate
file may be specified using the \verb|lmodel| qualifier.
...
{load\_xspec\_local\_models ( [dir [, pkg\_name]][;lmodel=filename])}
{build\_xspec\_local\_models }
...
associated package name. By default, model parameter
names are loaded from a text file named \verb|lmodel.dat|;
an alternate file may be specified using the \verb|lmodel| qualifier.
